//
//  RegisterNumberMatchDataManager.swift
//  Albazip
//
...
class RegisterNumberMatchDataManager {
    func getRegisterNumberMatch(vc: RegisterManagerInfoVC, number: String, name: String) {
        ...
        AF.request(str_url!, method: .get ,parameters: nil, encoding: JSONEncoding.default, headers: header)
            .validate()
            .responseDecodable(of: RegisterNumberMatchResponse.self) { response in
                switch response.result {
                case .success(let response):
                    vc.didSuccessRegisterNumberMatch(response)
