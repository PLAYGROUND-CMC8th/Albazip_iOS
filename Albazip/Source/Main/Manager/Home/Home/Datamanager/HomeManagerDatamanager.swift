...
class HomeManagerDatamanager {
    ...
    func getPostionCount(vc: HomeManagerVC) {
        let url = "\(Constant.BASE_URL)/home/positionCount"
        ...
        AF.request(url, method: .get ,parameters: nil, encoding: JSONEncoding.default, headers: header)
            .validate()
            .responseDecodable(of: PositionCountResponse.self) { response in
                switch response.result {
                case .success(let response):
                    switch response.code {
                    case "200":
                        vc.didSuccessPostionCount(result: response.data)
                    default:
                        vc.failedToRequestHomeManager(message: response.message!)
                    }
